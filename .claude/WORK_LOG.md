...
**후속 — 브라우저 유휴 종료 추가 (2026-08-27)**

재시작 후 검증 중 발견: 브라우저 재사용은 정상 동작하나(인스턴스 1개 확인),
**쓰지 않는 동안에도 크롬이 413MB 를 붙들고 있었다.** 결과서는 하루 몇 건 뽑는 기능이다.

마지막 렌더링 후 10분이 지나면 브라우저를 닫는다. 다음 요청에서 다시 띄운다.
...
0.3초 차이를 400MB 상주와 바꾸지 않는다는 판단이다.
(62GB 중 37GB 여유라 압박은 아니었지만, 무기한 점유는 옳지 않다)

**검증**

| 항목 | 결과 |
|------|------|
| 골프 결과서 (id=91) | 200 · 449KB · 3쪽 · **0.59초** |
...
| 브라우저 인스턴스 | golf-backend 자식 크롬 **1개** (재사용 정상) |
| 유휴 종료 | 임시로 12초 설정해 발동 확인 후 10분으로 복원 |
