...
/**
 * 결과 필드명을 phase와 item으로 변환
 * 예: addressShoulderTiltMent -> { phase: 'address', item: 'shoulder_tilt' }
 */
export function parseResultFieldName(fieldName: string): { phase: string; item: string } | null {
  // camelCase를 분리
  ...
  for (const [phaseKey, phaseValue] of Object.entries(phaseMapping)) {
    if (fieldName.startsWith(phaseKey)) {
...
/**
 * DB 필드명에서 멘트 조회
 * 예: getCommentByFieldName('addressShoulderTilt', 1) -> '기울기가 잘 유지돼요!'
 */
 ...
 * 항목별 한글 라벨 매핑
 */
export const ITEM_LABELS: { [key: string]: string } = {
  // Address
  addressShoulderTilt: '어깨 기울기',
  addressStance: '스탠스',
  addressUpperBodyTilt: '상체 기울임',

  // Takeback
  takebackLeftShoulderRotation: '어깨 회전',
  takebackLeftArmFlexion: '왼팔 펴짐',
  takebackRightArmFlexion: '오른팔 펴짐',
  takebackRightHipRotation: '골반 회전',

  // Backswing
  backswingHeadLocation: '머리 위치',
  backswingLeftShoulderRotation: '어깨 회전',
  backswingLeftArmFlexion: '왼팔 펴짐',

  // Backswingtop
  backswingTopReverseSpine: '리버스 스파인',
  backswingTopRightLegFlexion: '오른다리 펴짐',
  backswingTopHeadLocation: '머리 위치',
  backswingTopRightHipRotation: '골반 회전',
  backswingTopCenterOfGravity: '무게 중심',

  // Downswing
  downswingCenterOfGravity: '무게 중심',
  downswingRightElbowLocation: '오른팔꿈치 위치',
  downswingRightArmRotation: '오른팔 회전',

  // Impact
  impactHeadLocation: '머리 위치',
  impactHangingBack: '행잉백',
  impactLeftArmFlexion: '왼팔 펴짐',
  impactRightArmFlexion: '오른팔 펴짐',

  // Follow
  followLeftLineAlign: '왼다리 정렬',
  followChickenWing: '치킨윙',
  followCenterOfGravity: '무게 중심',

  // Finish
  finishCenterOfGravity: '무게 중심',
  finishRightFootRotation: '오른발 회전',
  finishLeftFootFix: '왼발 고정',
};

/**
 * 점수 기반 상태 결정
 */
export function getScoreStatus(score: number | null | undefined): 'good' | 'warning' | 'bad' {
...
export interface AnalysisFeedback {
  strengths: { label: string; comment: string; score: number }[];
  weaknesses: { label: string; comment: string; score: number }[];
}

export function classifyAnalysisResults(result: any): AnalysisFeedback {
  ...
  const scoreFields = [
    'addressShoulderTilt',
    'addressStance',
    'addressUpperBodyTilt',
    'takebackLeftShoulderRotation',
    'takebackLeftArmFlexion',
    'takebackRightArmFlexion',
    'takebackRightHipRotation',
    'backswingHeadLocation',
    'backswingLeftShoulderRotation',
    'backswingLeftArmFlexion',
    'backswingTopReverseSpine',
    'backswingTopRightLegFlexion',
    'backswingTopHeadLocation',
    'backswingTopRightHipRotation',
    'backswingTopCenterOfGravity',
    'downswingCenterOfGravity',
    'downswingRightElbowLocation',
    'downswingRightArmRotation',
    'impactHeadLocation',
    'impactHangingBack',
    'impactLeftArmFlexion',
    'impactRightArmFlexion',
    'followLeftLineAlign',
    'followChickenWing',
    'followCenterOfGravity',
    'finishCenterOfGravity',
    'finishRightFootRotation',
    'finishLeftFootFix',
  ];

  for (const field of scoreFields) {
    const scoreKey = `${field}Score`;
    const mentKey = `${field}Ment`;

    const score = result[scoreKey];
    const ment = result[mentKey] as 1 | 2 | 3;

    if (score === null || score === undefined || ment === null || ment === undefined) continue;

    const label = ITEM_LABELS[field] || field;
    const comment = getCommentByFieldName(field, ment);

    if (ment === 1) {
      feedback.strengths.push({ label, comment, score });
    } else {
      feedback.weaknesses.push({ label, comment, score });
    }
  }

  // 점수 기준 정렬 (강점: 높은 순, 약점: 낮은 순)
  feedback.strengths.sort((a, b) => b.score - a.score);
  feedback.weaknesses.sort((a, b) => a.score - b.score);
